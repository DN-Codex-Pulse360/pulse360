...
evidence, weighted attribute, and GPT-confidence skeleton for Pulse360.
...
OpenAI mode when `OPENAI_API_KEY` is available through Databricks secrets.
Live mode calls the OpenAI Responses API with strict Structured Outputs,
...
8. `25_weighted_attribute_resolution.sql`
9. `30_account_genai_enrichment_output.sql`
...
- Uses fixture mode unless `OPENAI_API_KEY` is available from the `pulse360-ai`
  Databricks secret scope and `--fixture-only` is omitted.
...
- Served attribute values must carry source contribution, source weight,
  freshness, conflict count, license reference, and run metadata.
